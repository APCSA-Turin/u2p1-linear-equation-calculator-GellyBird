...
public class LinearCalculator{
    ...
    public LinearCalculator(String coord1, String coord2){ // <--add 2 string parameters to this constructor
        int commaIndx1 = coord1.indexOf(",");
        int commaIndx2 = coord2.indexOf(",");
        x1 = Integer.parseInt(coord1, 1,commaIndx1,10);
        y1 = Integer.parseInt(coord1, commaIndx1+1,coord1.length()- 1,10);
        x2 = Integer.parseInt(coord2, 1,commaIndx2,10);
        y2 = Integer.parseInt(coord2, commaIndx2+1,coord2.length()- 1,10);
    }
    ...
    //yInt() -> returns a double. y=mx+b --> y-mx = b
    ...
    public double slope(){
        if ((x2-x1) == 0){
            ...
            double slope = (double)(y2-y1)/(x2-x1); // I casted (y2-y1) to a double because if if I dont it sometimes returns 0.0 for certain numbers
